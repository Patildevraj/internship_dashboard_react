...
const StudentTable = () => {
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        console.log('submitting', studentForm);
        let {id, name, classname, age} = studentForm;
        if (id.length > 0 && name.length > 0 && classname.length > 0) {
            setstudentinfo([...studentinfo, { id, name, classname, age }])
            handleClose();
            setStudentForm({
                id: '',
                name: '',
                            ...
                            <tbody id="tabledata">
                                {studentinfo.map((ele, index) => {
                                    return <tr className='text-success' key={ele.id}>
                                        ...
                                        <td>{ele.classname}</td>
                                        <td>
                                            <button className='btn btn-outline-primary me-3' onClick={()=> handleEdit(ele.id)}>Edit</button>
